...
modelCMR = function() {
  ...
  for (i in 1:M){
    ...
    for (t in 2:n.occasions){
      ...
      y[i, t] ~ dcat(po[z[i, t], t - 1, , group[i] + 1])
    } #t
  } #i
  ...
  for (i in 1:M) {
    for (t in 2:deadstop[i]) {
      al[i, t-1] <- equals(z[i, t], 2)
    } #t
    for (t in (deadstop[i]+1):n.occasions) {
      al[i, t-1] = 0
    }
    for (t in 1:(deadstop[i] - 1)) {
      d[i, t] <- equals(z[i, t] - al[i, t], 0)
    } #t
    for (t in (deadstop[i]):n.occasions) {
      d[i, t] = 0
    }
    alive[i] <- sum(al[i,])
  } #i
...
simul.js <- function(PHI, P, b, N){
  ...
  CH <- CH[-never,]
  first <- apply(CH[,1:ncol(CH)], 1, function(x) min(which(x == 1)))
  firstdead = which(first == Inf)
  CH = CH[-firstdead,]
...
last.one  <- apply(my.z.init[,1:ncol(CH.du)], 1, function(x) max(which(x !=0)))

for(i in 1:nrow(my.z.init)) {
  # if (my.z.init[i,last.one[i]]==2){last.one[i]=last.one[i]-1}
...
deadstop = apply(my.z.init.ms[,1:ncol(CH.ms)], 1, function(x) max(which(x == 1 | x==2)))
```
...
CMR=read.csv2("donneesCMR.csv")

#Test pour moins de temps
#CMR= CMR %>% as_tibble() %>% filter(!row_number() %in% seq(1,2392,2)) %>% filter(!row_number() %in% seq(1,2392,2)) %>% filter(!row_number() %in% seq(1,2392,2)) 
#
...
first.one <- apply(z.init[,1:ncol(CH.du)], 1, function(x) min(which(x != 0)))
last.one  <- apply(z.init[,1:ncol(CH.du)], 1, function(x) max(which(x != 0)))
# last.one  <- apply(z.init[,1:ncol(CH.du)], 1, function(x) max(which(x == 1 | x==2)))

for(i in 1:nrow(z.init)) {
  z.init[i, first.one[i]  : last.one[i]] = 2                           
  # 2 indique que l'on est sûr que l'individu est vivant
  if(first.one[i] > 1) z.init[i,1  : (first.one[i] - 1) ] = 1          
  # 1 indique que l'on n'a toujours pas observé l'individu (pas observé ou pas encore né)
  if(last.one[i] < ncol(z.init)) z.init[i, (last.one[i] + 1) : ncol(z.init)    ] = 3 
  # 3 indique que l'on n'observe plus l'individu (pas observé ou mort)
}
```
...
CH.ms[CH.ms == 2] = 1
CH.ms[CH.ms == 0] = 2             # Not seen = 2, seen = 1
## Il y avait déjà des 2 dans les données, indiquant qu'un individu est mort. Cette information supplémentaire est mise de côté avec ce modèle

z.init.ms = rbind(z.init, aug)
z.init.ms[z.init.ms == 0] = 1

deadstop = apply(z.init.ms[,1:ncol(z.init.ms)], 1, function(x) max(which(x=1 | x==2)))
```
...
```{r}
harvest <- c(0,0,0,0,1,0,0,2,1,2,0,0,1,0,4,4,6,18,36,34,42,51,98,105,103,169,207)
```
