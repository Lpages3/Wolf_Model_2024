...
ggplot() +
  ...
  labs(
    ...
    subtitle = "en bleu: modèle exponentiel ; en rouge: modèle logistique",
    x = "Years",
    y = "DIC") +
  theme_bw()
```
...
```{r}
for (t in 1:(nyears-1)){
   ssm_sim1$N[t+1] <- round(rnorm(1,ssm_sim1$N[t] * lambda,sigma)) - harvest[t]
}
...
Initialisation des données :
```{r}
bugs.data = list(nyears = nrow(ssm_sim1)+10,
                 y = c(ssm_sim1$y,rep(NA,10)))
```
...
sim_modelexp$BUGSoutput$sims.matrix %>%
  ...
  geom_ribbon(aes(x = years, ymin = lq, ymax = hq), fill = "red", alpha = 0.3)+
  geom_point(data = bugs.data %>% as_tibble, aes(x = 1995 + 1:unique(nyears), y = y)) + 
  coord_cartesian(xlim=c(1996,2030),ylim=c(0,5500))+
  theme_bw()+
...
for (nyears in seq(2,nyears,pas)) { # Boucle sur le nombre de tranches d'années parcourues
  ...
  # Début de l'estimation par approche bayésienne
    ...
    if (lambda < 1.2) {H = 0}
    if (lambda >= 1.2 & lambda < 1.3) {H = 0.2}
    if (lambda >= 1.3 & lambda < 1.4) {H = 0.3}
    if (lambda > 1.4) {H = 0.4}
...
ssm_sim6 = data.frame(Year = 1:nyears,
                      y = numeric(nyears),
                      N = numeric(nyears),
                      Nbis = numeric(nyears),
                      ybis = numeric(nyears))
...
for (nyears in seq(2, nyears, pas)) {
  ...
  if (nyears < 16) {
    ...
    if (nyears <= 5) {
      ...
      for (t in 1:(nyears - 1)) {
        ...
        ssm_sim6$N[t + 1] = rpois(1, u)
        ssm_sim6$Nbis[t + 1] = rpois(1, u)
      }
    }
    ...
    if (nyears > 5) {
      ...
      for (t in (nyears - 5):(nyears - 1)) {
        u = ssm_sim6$N[t] * (lamb - H)
        v = ssm_sim6$Nbis[t] * (lamb - NAMharvest)
        
        ssm_sim6$N[t + 1] = rpois(1, u)
        ssm_sim6$Nbis[t + 1] = rpois(1, v)
      }
    }
  }
  ...
  if (nyears >= 16) {
    for (t in (nyears - pas):(nyears - 1)) {
      u = ssm_sim6$N[t] * (1 - H)             # Taux de prélevement adaptatif
      v = ssm_sim6$Nbis[t] * (1 - NAMharvest) # Taux de prélevement constant
      
      Er = exp(alpha * (1 - c(u, v) / K)) * c(u, v)
      
      ssm_sim6$N[t + 1] = rpois(1, Er[1])
      ssm_sim6$Nbis[t + 1] = rpois(1, Er[2])
    }
  ...
  # Simulation des effecitfs observés
  for (t in 1:nyears) {
    ssm_sim6$y[t] = rpois(1, ssm_sim6$N[t])
    ssm_sim6$ybis[t] = rpois(1, ssm_sim6$Nbis[t])
  }
  
  # Début de l'estimation par approche bayésienne
    # Initialisation des données
    bugs.data = list(nyears = nyears,
                     y = c(ssm_sim6$y[1:nyears],rep(NA,5)),
                     dH = H)
...
wolf_modellogist$BUGSoutput$sims.matrix%>%
  ...
  geom_line(aes(x = years, y = medianN), colour = "red", lty = "dashed")+
  geom_ribbon(aes(x = years, ymin = lq, ymax = hq), fill = "red", alpha = 0.3)+
  geom_point(data = bugs.data %>% as_tibble, aes(x = 1:unique(nyears+5), y = c(ssm_sim6$y,rep(NA,5)))) + 
  theme_bw()+
  labs(title = "Estimated population size",
       subtitle = "Observed population size (black dots)",
       x = "Years",
...
# Initialisation des données
bugs.data = list(nyears = nyears,
                 y = c(ssm_sim6$ybis[1:nyears], rep(NA, 5)),
                 dH = NAMharvest)
...
output = output1 %>% left_join(output2)
  ...
  ggplot(output) +
  geom_point(data = ssm_sim6, aes(x = seq(1,27), y = y), colour = "red") +
  geom_point(data = ssm_sim6, aes(x = seq(1, 27), y = ybis), colour = "blue")+
