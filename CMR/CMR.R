...
CH <- sim$CH
# colnames(CH) = seq(1996,1996+n.occasions-1)
...
# colnames(aug) = c("dummy",seq(1996,1996+n.occasions-1))
...
my.z.init.ms <- rbind(my.z.init, aug)
...
CMRBayesian=modelCMR$BUGSoutput

a= modelCMR$BUGSoutput$sims.matrix %>%
  as_tibble() %>%
...
#   geom_point(data = bugs.data %>% as_tibble, aes(x = 1995 + 1:unique(n.occasions), y = y)) + 
